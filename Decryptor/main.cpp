...
{
    if (argc != 3)
        return cout << "Program needs TWO arguments, input and output!" << std::endl, 2;
    string in (argv[1]), out (argv[2]);
    std::ifstream fi (in);
    if (!fi)
        return std::cout << "Cannon read input file '" << in << "'" << std::endl, 3;
    std::string data;
    fi >> data;
    if (!fi)
...
std::string DecryptB64 (std::string s)
{
        ...
        s = s.erase (1, 1);
        s = base64_decode (s);
        s = s.substr (SALT2.length()-1 + SALT3.length()-1);
